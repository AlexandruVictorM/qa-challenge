//#----- Test Procedure -----#
describe('Test 2 = add NRC Quest to favorites and check if it remains there', () => {
  before(() => {
    cy.visit('http://localhost:3000/')
  })

it("Find and click favorites, and check if the click  has an effect", () => { 
cy.get("[data-cypress='favorite-NRC Quest']").should('exist')
    .should('have.attr','title')
    .and('contain', 'Add to favourites')
cy.get("[data-cypress='favorite-NRC Quest']").first().click()
cy.get("[data-cypress='favorite-NRC Quest']")
    .should('have.attr','title')
