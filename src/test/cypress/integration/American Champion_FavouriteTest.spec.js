//#----- Test Procedure -----#
describe('Test 2 = add American Champion to favorites and check if it remains there', () => {
  before(() => {
    cy.visit('http://localhost:3000/')
  })

it("Find and click favorites, and check if the click  has an effect", () => { 
cy.get("[data-cypress='favorite-American Champion']").should('exist')
    .should('have.attr','title')
    .and('contain', 'Add to favourites')
cy.get("[data-cypress='favorite-American Champion']").first().click()
cy.get("[data-cypress='favorite-American Champion']")
    .should('have.attr','title')
