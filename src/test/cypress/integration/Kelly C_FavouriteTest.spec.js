//#----- Test Procedure -----#
describe('Test 2 = add Kelly C to favorites and check if it remains there', () => {
  before(() => {
    cy.visit('http://localhost:3000/')
  })

it("Find and click favorites, and check if the click  has an effect", () => { 
cy.get("[data-cypress='favorite-Kelly C']").should('exist')
    .should('have.attr','title')
    .and('contain', 'Add to favourites')
cy.get("[data-cypress='favorite-Kelly C']").first().click()
cy.get("[data-cypress='favorite-Kelly C']")
    .should('have.attr','title')
