//#----- Test Procedure -----#
describe('Test 2 = add Pacific Freeedom to favorites and check if it remains there', () => {
  before(() => {
    cy.visit('http://localhost:3000/')
  })

it("Find and click favorites, and check if the click  has an effect", () => { 
cy.get("[data-cypress='favorite-Pacific Freeedom']").should('exist')
    .should('have.attr','title')
    .and('contain', 'Add to favourites')
cy.get("[data-cypress='favorite-Pacific Freeedom']").first().click()
cy.get("[data-cypress='favorite-Pacific Freeedom']")
    .should('have.attr','title')
